...
module.exports = {
  ...
  async cadastrarUsuario(req, res) {
    const { nome, email, senha, cpf, cidade, rua, numero, bairro, cep, telefone, tipo_usuario } = req.body
  ...
  async validarUsuario(req, res, next) {
    ...
    const [, resposta] = await Usuario.update(
      { verificado: true },
      { returning: true, where: { email: tokenDecodificado.email } }
    )
